...
def build_source_pattern(source_prefix):
    return f"{source_prefix}%.csv.gz"
...
def get_flights_by_source_file(cursor, source_pattern):
    sql = f"""
        select
            source_file,
            count(*) as rows_cnt
    ...
    rows = fetch_all(cursor, sql, (source_pattern,))

    result = {}

    for source_file, rows_cnt in rows:
        result[source_file] = rows_cnt

    return result


def check_flights_raw(cursor, source_prefix):
    source_pattern = build_source_pattern(source_prefix)

    rows_by_source_file = get_flights_by_source_file(cursor, source_pattern)

    if not rows_by_source_file:
        raise ValueError("flights_raw has no rows for source prefix")
    ...
    row = fetch_one(cursor, summary_sql, (source_pattern,))
    ...
    logger.info("flights source prefix: %s", source_prefix)
    logger.info("flights source files: %s", len(rows_by_source_file))
    ...
    for source_file in sorted(rows_by_source_file):
        logger.info(
            "flights file rows: source_file=%s rows=%s",
            source_file,
    ...
    return rows_by_source_file


def check_load_log_for_file(cursor, source_file, rows_cnt):
    success_count_sql = f"""
        select count(*)
        from {LOG_TABLE}
        where flow_name = %s
    ...
    logger.info("success log rows for %s: %s", source_file, success_count)

    if success_count == 0:
        raise ValueError(f"etl_load_log has no success row for {source_file}")

    if success_count > 1:
        raise ValueError(f"etl_load_log has more than one success row for {source_file}")
    ...
    logger.info("log rows_loaded for %s: %s", source_file, rows_loaded)
    logger.info("log status for %s: %s", source_file, status)
    logger.info("log error_message for %s: %s", source_file, error_message)

    if rows_loaded != rows_cnt:
        raise ValueError(f"rows_loaded does not match flights_raw rows for {source_file}")

    if error_message is not None:
        raise ValueError(f"success log row has error_message for {source_file}")


def check_load_logs(cursor, rows_by_source_file):
    for source_file in sorted(rows_by_source_file):
        check_load_log_for_file(
            cursor=cursor,
            source_file=source_file,
            rows_cnt=rows_by_source_file[source_file],
        )


def check_airport_links(cursor, source_prefix):
    source_pattern = build_source_pattern(source_prefix)
    ...
    missing_origin_cnt = fetch_one(cursor, origin_sql, (source_pattern,))[0]
    missing_dest_cnt = fetch_one(cursor, dest_sql, (source_pattern,))[0]
...
def run_checks(source_prefix):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            rows_by_source_file = check_flights_raw(cursor, source_prefix)
            check_load_logs(cursor, rows_by_source_file)
            check_airport_links(cursor, source_prefix)
    finally:
        connection.close()
...
def main():
    ...
    source_prefix = build_source_prefix(args.flights_prefix, args.flight_date)
    run_checks(source_prefix)
